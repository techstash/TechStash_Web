...
@Repository
public class ResourceSectionDAOImpl implements ResourceSectionDAO {
	...
	public List<ResourceSection> getImage(int id) {
		...
		Query theQuery=currentSession.createQuery("from ResourceSection where id=:id");
		theQuery.setParameter("id", id);
		List<ResourceSection> result = theQuery.getResultList();
	...
	public List<ResourceSection> getResultWebsite() {
		byte[] image=null;
		Session currentSession = entityManagerFactory.unwrap(SessionFactory.class).openSession();
		Query theQuery = currentSession.createQuery("from ResourceSection where id=1",ResourceSection.class);
		List<ResourceSection> result = theQuery.getResultList();
	...
	public ResourceSection getResultAdmin() {
		Session currentSession = entityManagerFactory.unwrap(SessionFactory.class).openSession();
		int id=1;
		ResourceSection dbresult=currentSession.get(ResourceSection.class,id);
		currentSession.close();
		return dbresult;
	}
	...
	public void resourceSectionUpdate(byte[] image, String title, String subtitle, String buttontext, boolean status) {
		...
		Query theQuery = currentSession.createNativeQuery("UPDATE resourcesection set image=:image,title=:title,subtitle=:subtitle,buttontext=:buttontext,status=:status where id=1");
